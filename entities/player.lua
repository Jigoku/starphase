...
function player:init(playersel)
	...
	player.x = starfield.w/2-player.gfx:getWidth()/2
	player.y = starfield.h-player.gfx:getHeight()*3
...
function player:fireCannon(dt)
	...
	if self.cannon.cycle <= 0 then
		...
		table.insert(projectiles.missiles, {
			...
			x = self.x + self.gfx:getWidth()/2-projectiles.cannon.gfx:getWidth()/2 +(player.cannon.switch and -35 or 35),
			y = self.y - projectiles.cannon.gfx:getHeight()/2,
			yvel = 2000,
			xvel = 0,
...
function player:fireBlaster(dt)
	...
	if self.blaster.cycle <= 0 then
		...
		table.insert(projectiles.missiles, {
			...
			x = self.x + self.gfx:getWidth()/2-(projectiles.blaster.gfx:getWidth()/2),
			y = self.y + self.gfx:getHeight()/2,
			yvel = 1250,
			xvel = 0,
...
function player:fireWave(dt)
	...
	if self.wave.cycle <= 0 then
		...
		table.insert(projectiles.missiles, {
			...
			x = self.x + self.gfx:getWidth()/2-(projectiles.wave.gfx:getWidth()/2),
			y = self.y + self.gfx:getHeight()/2,
			switch = true,
			yvel = 750,
			xvel = 0,
...
function player:firePlasma(dt)
	...
	if self.plasma.cycle <= 0 then
		...
		table.insert(projectiles.missiles, {
			...
			x = self.x + self.gfx:getWidth()/2-projectiles.plasma.gfx:getWidth()/2 +(player.plasma.switch and -28 or 28),
			y = self.y + self.gfx:getHeight()/2,
			yvel = 750,
			xvel = 0,
...
function player:fireOrb(dt)
	...
	if self.orb.cycle <= 0 then
		...
		table.insert(projectiles.missiles, {
			...
			x = self.x + self.gfx:getWidth()/2-projectiles.orb.gfx:getWidth()/2 +(player.orb.switch and -28 or 28),
			y = self.y + self.gfx:getHeight()/2,
			yvel = 800,
			xvel = (player.orb.switch and 200 or -200),
...
function player:fireRocket(dt)
	...
	if self.rocket.cycle <= 0 then
		...
		table.insert(projectiles.missiles, {
			...
			x = self.x + (player.rocket.switch and self.gfx:getWidth()/2-(projectiles.rocket.gfx:getWidth()/2) or self.gfx:getWidth()/2+(projectiles.rocket.gfx:getWidth()/2)),
			y = self.y + self.gfx:getWidth()/2-(projectiles.rocket.gfx:getWidth()/2),
			switch = player.rocket.switch,
			trigger = 150,
			launched = false,
			yvel = 900,
			xvel = (player.rocket.switch and -400 or 400),
...
function player:fireBeam(dt)
	...
	if self.beam.cycle <= 0 and self.energy > 0 then
		...
		table.insert(projectiles.missiles, {
			...
			x = self.x + self.gfx:getWidth()/2-(projectiles.beam.gfx:getWidth()/2),
			y = self.y + projectiles.beam.gfx:getHeight(),
			yvel = 700,
			xvel = 0,
